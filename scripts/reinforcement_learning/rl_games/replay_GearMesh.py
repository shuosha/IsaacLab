...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    input_path = "rrl_data/gearmesh_train_data"
    ...
    for i, ep in enumerate(eps_idx):
        ...
        held2base_pos[i][2] -= 0.014739 # or 0.010709
        ...
        fixed2base_pos[i][2] -= 0.008 # if z is flipped
        fixed2base_pos[i][0] += 0.005
        
        q = quat_from_matrix(fixed2base_mat[:3, :3]).to(env.device)
        qy = torch.tensor([0., 0., 1., 0.], device=q.device, dtype=q.dtype)  # 180° about Y
        q_flip_y = quat_mul(q, qy)   # apply 180° about local Y
        fixed2base_quat[i] = q_flip_y
