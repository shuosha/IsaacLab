...
class FactoryEnvDexGen(DirectRLEnv):
    ...
    def _pre_physics_step(self, action):
        ...
        ctrl_target_fingertip_midpoint_quat = self.actions[:, 3:7]
    ...
    def _apply_action(self):
        ...
        ctrl_target_gripper_dof_pos = self.actions[:, 7:8].clone() * 1.6
    ...
    def _get_rewards(self):
        ...
        # -------------------------
        action_norm = torch.norm(self.actions, dim=1) / math.sqrt(8)

        # -------------------------
        # Action-smoothing penalty (soft)
        # -------------------------
        action_smoothing = torch.norm(self.prev_actions - self.actions, dim=1)
        ...
        rew_dict = {
            ...
            "terminated": -(self.reset_buf & (~self.eps_task_succeeded)).float() * self.cfg.env_options.termination_reward_scale,
